...
if ("BEST_NOW".equals(parsed.intent)) {
  // default priorities untuk best offer now
  ...
  private String saveRequest(String text, NLP.Intent parsed, List<Rec> recs) throws ExecutionException, InterruptedException {
  ...
  private String str(DocumentSnapshot d, String k){ return Optional.ofNullable(d.getString(k)).orElse(""); }
  private double dbl(DocumentSnapshot d, String k){
    Double v = d.getDouble(k); if (v != null) return v;
    Long l = d.getLong(k); return l == null ? 0.0 : l.doubleValue();
  }
  private long lng(DocumentSnapshot d, String k){ Long v = d.getLong(k); return v == null ? 0L : v; }
}
